...
- **Source**: 1장 시스템 특징, Tizen Platform · Web Engine Team (3.2.2)
...
- **Description**: 본 시스템은 독립 실행뿐 아니라, Tizen Platform · Web Engine Team이 소유한 Platform Main Agent **argot(argo-tizen)** 의 **Sub-Agent / Skill**로도 호출 가능해야 한다. argot은 AIOS(Tizen OS AI화)의 메인 에이전트로서 Bixby를 통해 사용자 질의를 수신하고, 브라우저 제어 태스크로 판단 시 본 시스템을 호출하며, 본 시스템의 결과를 받아 사용자에게 표시한다 (호출 흐름은 [2장 시스템 컨텍스트](02-project-overview.md) 참조). 본 시스템의 공개 인터페이스는 argot의 호출 계약과 호환되어야 하며, argot의 컨텍스트·세션 모델을 침해하지 않아야 한다.
- **Source**: Tizen Platform · Web Engine Team (3.2.2)
...
- **Description**: 본 시스템은 내부 플랫폼(argot)뿐 아니라, **외부 에이전트 오케스트레이션 플랫폼**(MCP 클라이언트, Agent Protocol 등)에서도 Sub-Agent / Skill로 호출 가능하도록 표준화된 공개 인터페이스를 제공해야 한다. 인터페이스 표준은 MCP tool spec 또는 OpenAPI schema 중 하나 이상을 지원한다.
...
### OC-003 Tizen Platform · Web Engine Team의 인터페이스·격리 정책

- **Description**: Tizen Platform · Web Engine Team은 본 시스템의 호스트이자 가장 가까운 내부 소비자다. 본 시스템이 노출하는 공개 인터페이스의 안정성·버저닝, 그리고 본 시스템의 실패가 Main Agent / GenUI Renderer로 전파되지 않는 격리 전략은 Platform Team이 정의한 정책을 따라야 한다.
- **Source**: Tizen Platform · Web Engine Team (3.2.2)
...
### OC-004 보안 · 프라이버시 · 컴플라이언스의 보안 게이트

- **Description**: 자격증명 보관·사용, PII 데이터 흐름, 권한 모델, 위협 모델은 보안 · 프라이버시 · 컴플라이언스의 사전 심사·승인을 받아야 한다. 본 시스템의 설계는 해당 팀이 정의한 권한 도메인·트러스트 경계·암호화 표준 안에서 이루어져야 한다.
...
- **Source**: 접근성 · 고령 사용자 (3.4.2), W3C ARIA
...
|----|------|--------|-------------|
| TC-001 | Chromium 기반 Web Engine 및 CDP 의존 | 3.2.2 | 페이지 인식 메커니즘(7장), Component View |
| TC-002 | On-device 에이전트 하네스 + Cloud LLM | 1장, 3.2.2 | NFR-008, NFR-018 |
| TC-003 | 외부 ASR / 화자 식별 시스템 의존 | 3.2.2 | FR-001 |
| TC-004 | 라인업별 HW 스펙 차이 | 3.2.1, 3.2.2 | NFR-006, NFR-007, NFR-011 |
| TC-005 | Platform Main Agent(argot)의 Sub-Agent 동작 | 3.2.2 | FR-025, FR-026, FR-027 |
| TC-006 | 외부 Agent 플랫폼 호출 표준 인터페이스 | 3.3.2 | FR-025, FR-026, FR-027, NFR-023 |
| OC-001 | VD 사업부 상품화 게이트 | 3.2.1 | 전반 |
| OC-002 | LLM API 호출 비용 예산 | 3.2.1 | NFR-008, FR-031 |
| OC-003 | Tizen Platform · Web Engine Team 인터페이스·격리 정책 | 3.2.2 | NFR-009, NFR-023 |
| OC-004 | 보안 · 프라이버시 · 컴플라이언스 보안 게이트 | 3.2.5 | NFR-012, NFR-014, NFR-015, NFR-017, NFR-018 |
| OC-005 | LLM 토큰 사용 비용 사용자 미부과 | 3.2.1 | NFR-008, FR-031 |
| CN-001 | Tizen TV 디자인 시스템 (10-feet UI) | 3.2.3 | Generative UI View |
| CN-002 | ARIA / 시맨틱 마크업 표준 | 3.4.2, W3C | NFR-022 |
