...
- **Description**: 에이전트 루프(도구 선택·실행 흐름·HITL 흐름) 자체는 TV 디바이스 안에서 실행되며, 추론(reasoning)은 외부 클라우드 LLM API 직접 호출로 수행한다. 본 시스템은 on-device 추론을 1차 목표로 하지 않으며, 별도의 모델 추상화 계층은 두지 않는다. (특정 보조 작업에서 on-device SLM 가용성은 별도 결정 사안.)
- **Source**: 1장 시스템 특징, 웹 엔진 팀 (3.2.3)
...
### TC-005 Platform Main Agent(argot)의 Sub-Agent로 동작 가능

- **Description**: 본 시스템은 독립 실행뿐 아니라, ARGO 개발팀이 소유한 Platform Main Agent **argot(argo-tizen)** 의 **Sub-Agent / Skill**로도 호출 가능해야 한다. argot은 AIOS(Tizen OS AI화)의 메인 에이전트로서 Bixby를 통해 사용자 질의를 수신하고, 브라우저 제어 태스크로 판단 시 본 시스템을 호출하며, 본 시스템의 결과를 받아 사용자에게 표시한다 (호출 흐름은 [2장 시스템 컨텍스트](02-project-overview.md) 참조). 본 시스템의 공개 인터페이스는 argot의 호출 계약과 호환되어야 하며, argot의 컨텍스트·세션 모델을 침해하지 않아야 한다.
...
- **Description**: 본 시스템은 내부 플랫폼(argot)뿐 아니라, **외부 에이전트 오케스트레이션 플랫폼**(MCP 클라이언트, Agent Protocol 등)에서도 Sub-Agent / Skill로 호출 가능하도록 표준화된 공개 인터페이스를 제공해야 한다. 인터페이스 표준은 MCP tool spec 또는 OpenAPI schema 중 하나 이상을 지원한다. 외부 에이전트 플랫폼 자체는 본 시스템과 직접 상호작용이 미확정이므로 Stakeholder가 아닌 제약으로 취급하며(3.6), 표준 인터페이스 요구는 ARGO 개발팀이 참조 소비자로 대표한다.
- **Source**: ARGO 개발팀 (3.2.2, 참조 소비자) — cf. 3.6 Out-of-Scope(외부 Agent 플랫폼)
...
- **Description**: 본 시스템의 운영 비용 중 가장 큰 비중은 Cloud LLM API 호출 비용이다. VD 사업부(상품화)는 사용량 규모 대비 비용 예측 가능성을 출시 승인 전제 조건으로 요구한다. 세션당 평균 토큰 사용량 상한, 비용 모니터링·알림 체계, 비용 폭주 시 강제 차단 정책이 사전에 정의되어야 한다.
- **Source**: VD 사업부 (3.2.1)
...
### OC-003 ARGO 개발팀의 호출 계약·격리 정책

- **Description**: ARGO 개발팀은 본 시스템의 가장 가까운 내부 소비자(호출자)다. 본 시스템이 노출하는 공개 인터페이스의 안정성·버저닝, 그리고 본 시스템의 실패가 Main Agent / GenUI Renderer로 전파되지 않는 격리 전략은 ARGO 개발팀이 정의한 호출 계약·격리 정책을 따라야 한다.
- **Source**: ARGO 개발팀 (3.2.2)
...
### OC-004 보안 · 컴플라이언스팀의 보안 게이트

- **Description**: 자격증명 보관·사용, PII 데이터 흐름, 권한 모델, 위협 모델은 보안 · 컴플라이언스팀의 사전 심사·승인을 받아야 한다. 본 시스템의 설계는 해당 팀이 정의한 권한 도메인·트러스트 경계·암호화 표준 안에서 이루어져야 한다.
...
- **Description**: Cloud LLM API 호출에 따른 토큰 사용 비용은 Samsung이 부담하며, 최종 사용자에게 별도 과금하지 않는다. 본 시스템 사용에 사용량 기반 결제 모델은 존재하지 않으며, 사용 빈도가 사용자 부담으로 직결되지 않는다. 이는 본 시스템의 운영 비용 관리 부담이 전적으로 Samsung 내부 예산으로 흡수됨을 의미하며, 비용 최적화(컨텍스트 압축·캐싱 등)는 사용자 가치 보호가 아닌 **사업 지속가능성 차원의 요구**가 된다.
- **Source**: VD 사업부 (3.2.1)
...
- **Description**: 본 시스템이 출력하는 모든 UI(Generative UI 포함)는 Tizen TV 디자인 시스템의 토큰(타이포그래피·색상·포커스·모션·간격)과 10-feet UI(원거리 시청) 가이드라인을 따라야 한다. UI 라이브러리·디자인 토큰은 UX팀이 소유·제공한다.
- **Source**: UX팀 (3.2.5)
...
|----|------|--------|-------------|
| TC-001 | Chromium 기반 Web Engine 및 CDP 의존 | 3.2.3 | 페이지 인식 메커니즘(7장), Component View |
| TC-002 | On-device 에이전트 하네스 + Cloud LLM | 1장, 3.2.3 | NFR-008, NFR-018 |
| TC-003 | 외부 ASR / 화자 식별 시스템 의존 | 3.2.3 | FR-001 |
| TC-004 | 라인업별 HW 스펙 차이 | 3.2.1, 3.2.3 | NFR-006, NFR-007, NFR-011 |
| TC-005 | Platform Main Agent(argot)의 Sub-Agent 동작 | 3.2.2 | FR-025, FR-026, FR-027 |
| TC-006 | 외부 Agent 플랫폼 호출 표준 인터페이스 | 3.2.2 (참조 소비자) | FR-025, FR-026, FR-027, NFR-023 |
| OC-001 | VD 사업부 상품화 게이트 | 3.2.1 | 전반 |
| OC-002 | LLM API 호출 비용 예산 | 3.2.1 | NFR-008, FR-031 |
| OC-003 | ARGO 개발팀 호출 계약·격리 정책 | 3.2.2 | NFR-009, NFR-023 |
| OC-004 | 보안 · 컴플라이언스팀 보안 게이트 | 3.2.8 | NFR-012, NFR-014, NFR-015, NFR-017, NFR-018 |
| OC-005 | LLM 토큰 사용 비용 사용자 미부과 | 3.2.1 | NFR-008, FR-031 |
| CN-001 | Tizen TV 디자인 시스템 (10-feet UI) | 3.2.5 | Generative UI View |
| CN-002 | ARIA / 시맨틱 마크업 표준 | 3.3.2, W3C | NFR-022 |
