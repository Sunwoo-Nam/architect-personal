...
- **Measure Basis**: *(작성 예정)*
- **Rationale**: 의도 파악 실패는 잘못된 태스크 실행으로 직결되어 사용자 신뢰의 가장 큰 변수. 인식 정확도(NFR-020)와 분리 측정해야 원인 디버깅이 가능.
- **Related FR**: `FR-001`
- **Source**: 일반 시청자 (3.4.1)
- **Status**: Draft
...
- **Measure Basis**: *(작성 예정)*
- **Rationale**: 식별 실패는 잘못된 프로필 전환을 유발하며, 이는 곧 데이터 격리(NFR-016) 위반으로 이어짐. 가족 공유 환경의 프라이버시 보장 전제 조건.
...
|------|------|----------|------------|--------|
| NFR-001 | 간단한 명령의 End-to-end 응답 시간 | LAT | FR-001, FR-009 | 3.4.1 |
| NFR-002 | 복잡한 태스크 첫 피드백 시간 | LAT | FR-009 | 3.4.1 |
| NFR-003 | 진행 상황 표시 업데이트 주기 | LAT | FR-009 | 3.4.1 |
| NFR-004 | 외부 Skill 호출 인터페이스 응답 시간 | LAT | FR-040, FR-042 | 3.3.3 |
| NFR-005 | 슬립 복귀 후 에이전트 가용 시간 | LAT | FR-038 | 3.2.3, 3.4.1 |
| NFR-006 | 메모리 풋프린트 한도 | RES | FR-025 | 3.2.3, 3.2.2 |
| NFR-007 | CPU 점유율 한도 | RES | FR-011 | 3.2.3, 3.4.1 |
| NFR-008 | 세션당 토큰 비용 예산 | RES | FR-047 | 3.2.2, 3.2.8 |
| NFR-009 | 에이전트 오류의 상위 시스템 영향 격리 | AVL | FR-039 | 3.2.3 |
| NFR-010 | 네트워크 끊김 후 재개 성공률 | AVL | FR-012 | 3.4.1 |
| NFR-011 | 라인업별 기능 동등성 | AVL | FR-025 | 3.2.2 |
| NFR-012 | Web Runtime 샌드박스 경계 준수 | SEC | FR-034 | 3.2.3, 3.2.5 |
| NFR-013 | 자격증명 저장 암호화 강도 | SEC | FR-032 | 3.2.5 |
| NFR-014 | 프롬프트 인젝션 탐지율 | SEC | FR-035 | 3.2.5 |
| NFR-015 | 외부 호출자 인증 강도 | SEC | FR-042 | 3.3.3, 3.2.5 |
| NFR-016 | 프로필 간 데이터 격리 강도 | SEC | FR-029 | 3.4.4, 3.2.5 |
| NFR-017 | PII 데이터 기본 보존 기간 | PRV | FR-033, FR-036 | 3.2.5, 3.5 |
| NFR-018 | 클라우드 전송 데이터의 최소화 | PRV | FR-032, FR-037 | 3.2.5, 3.5 |
| NFR-019 | 데이터 주체 권리 행사 응답 시간 | PRV | FR-036 | 3.5 |
| NFR-020 | 음성 인식 정확도 | QLT | FR-001, FR-002 | 3.4.1, 3.4.2 |
| NFR-021 | 의도 파악 정확도 | QLT | FR-001 | 3.4.1 |
| NFR-022 | 태스크 완료 성공률 | QLT | FR-004, FR-005 | 3.4.1 |
| NFR-023 | 음성 화자 식별 정확도 | QLT | FR-027, FR-029 | 3.4.4 |
| NFR-024 | WCAG 2.2 AA 준수 | ACC | FR-020, FR-030 | 3.4.3, 3.5 |
| NFR-025 | 보조 기술 인터페이스 응답 시간 | ACC | FR-030 | 3.4.3 |
| NFR-026 | 공개 인터페이스 하위 호환성 유지 기간 | MNT | FR-041 | 3.3.3, 3.2.3 |
| NFR-027 | 모듈 결합도 | MNT | FR-049 | 3.2.8 |
| NFR-028 | 시뮬레이션 모드 시나리오 커버리지 | TST | FR-048 | 3.2.7, 3.2.8 |
| NFR-029 | Mock 인터페이스 적용 범위 | TST | FR-049 | 3.2.8 |
| NFR-030 | 에이전트 트레이스 보존 기간 | OBS | FR-046 | 3.2.8 |
| NFR-031 | 토큰·비용·지연 메트릭 노출 주기 | OBS | FR-047 | 3.2.8, 3.2.2 |
| NFR-032 | HITL 응답 후 태스크 재개 시간 | LAT | FR-006, FR-007 | 3.4.1, 3.2.6 |
| NFR-033 | 영구 저장소 풋프린트 한도 | RES | FR-025, FR-026, FR-033, FR-046 | 3.2.2, 3.2.3 |
...
4. **NFR-020 WER 벤치마크 셋** — 고령 사용자 발화 벤치마크 데이터셋 선정 필요.
5. **NFR-022 표준 시나리오 Top-100** — 시나리오 카탈로그 정의 필요 (시나리오 챕터에서 작성).
6. **NFR-033 영구 저장소 한도** — 라인업 등급별 디스크 예산. VD 상품화 담당자 + Tizen Platform Team 합의 필요.
