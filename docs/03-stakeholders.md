...
3. **Stakeholder는 시스템 외부의 상호작용 주체만이 아니라, 시스템의 형성·수용·운영에 이해관계를 가진 모든 역할을 포함한다.** 엔드유저뿐 아니라 내부 개발 조직, 외부 파트너, 규제 기관을 포함한다.
...
본 장 마지막의 3.6 절에서는 전체 Stakeholder와 Architectural View를 통합 매핑한 표를 제공하며, 이는 이후 5장 이후의 아키텍처 View 작성 범위를 결정하는 기준이 된다.
...
- **주요 Concern**
  ...
  - Cloud LLM API 호출 비용(토큰 과금 구조)과 라인업·사용량 규모별 운영 비용 예측 가능성
- **관련 Architectural View**
...
### 3.2.3 Tizen Platform Team (본 시스템의 내부 소비자)
...
- **주요 Concern**
  ...
  - Claw 계열뿐 아니라 **외부 에이전트 플랫폼(MCP, Agent Protocol 등)에서 본 시스템을 Skill / Sub-Agent로 호출**할 수 있도록 표준 인터페이스를 노출할 때의 보안 경계와 버저닝 정책
- **관련 Architectural View**
...
### 3.2.4 Web Engine Team (CDP · Browser Session 지원)
...
### 3.2.8 AI Web Agent 개발팀 (Internal Development Team)

- **역할**: 본 AI Web Agent를 실제로 구현·통합·배포·운영하는 내부 엔지니어링 팀. 아키텍트가 정의한 설계를 코드로 구현하는 주체이며, 일상적인 개발·디버깅·운영 사이클을 담당한다.
- **주요 Concern**
  - LLM API 호출 시 **토큰 사용량·응답 시간·비용**을 실시간으로 추적하고 최적화할 수 있는 디버깅·계측 도구의 존재
  - 에이전트 루프 실행 트레이스(도구 호출 순서, 컨텍스트 스냅샷, 에러 지점) 가시성
  - 비결정적(non-deterministic) AI 동작에 대해 **재현 가능한 테스트·시뮬레이션 환경** 구축 가능성
  - 온디바이스 하네스와 클라우드 LLM 간 레이턴시·오류율 모니터링을 개발 루프 안에서 즉시 확인 가능한지
  - 기능 영역(VUI, AGT, BRW 등)별 독립 개발·테스트가 가능한 모듈 경계와 Mock/Stub 지원
- **관련 Architectural View**
  - Observability View (토큰·레이턴시·에러 메트릭 설계)
  - Test Strategy View (AI 동작 재현성·시뮬레이션 전략)
...
### 3.3.3 외부 Agent 플랫폼 통합 개발자

- **역할**: 본 AI Web Agent를 자사 에이전트 오케스트레이션 플랫폼(예: LangChain, AutoGen, Claude Operator, MCP 클라이언트 등)에서 **Skill 또는 Sub-Agent로 호출**하려는 외부 개발자. Claw 계열 내부 플랫폼(3.2.3)과 달리, 본 시스템이 제공하는 **표준 공개 인터페이스**만을 통해 상호작용하는 제3자다.
- **주요 Concern**
...
- **주요 Concern**
  ...
  - **태스크 복잡도에 비례한 응답 속도**: 간단한 명령(채널 변경, 날씨 조회 등)은 즉각 처리되고, 복잡한 태스크(구매, 예약)에서도 단계별 피드백이 끊임없이 제공되는지
  - 의도와 다른 동작이 발생했을 때 이해 가능한 피드백과 쉬운 취소·재시도 경로
  - 원격조작 습관을 해치지 않는 일관된 포커스·확인 흐름
  - **Headless 결과 수신**: 굳이 브라우저 화면을 띄우지 않아도 되는 태스크(정보 조회, 예약 확인 등)는 결과만 화면에 표시되는 방식 선호
  - **로그인·캡차 등 인증 장벽 자동 처리**: 서비스 접근에 필요한 로그인, 캡차 통과 등을 에이전트 또는 플랫폼이 대신 처리하고, 사용자는 리모컨 없이 음성으로만 진행할 수 있는지
  - **에이전트와 직접 조작 간의 자연스러운 제어 전환**: 에이전트가 작업 중일 때도 사용자가 직접 페이지를 탐색·조작할 수 있고, 전환이 충돌 없이 자연스럽게 이루어지는지 (co-navigation)
  - **구매·예약 트랜잭션의 완전 자동 완료**: "맥북 가장 싼 거 구매해줘" 수준의 요청을 실제 결제 완료까지 에이전트가 이어서 처리하는지
  - **판단·선택이 필요한 시점에 사용자 확인 요청**: 에이전트가 불확실한 판단을 스스로 내리지 않고, 선택지를 제시하거나 명확화 질문을 통해 사용자 의지를 반영하는지
  - **범용 태스크 처리 능력**: 단일 에이전트로 검색·조회·예약·구매·콘텐츠 탐색·스마트홈 제어 등 다양한 도메인 작업을 수행할 수 있는지
  - **히스토리·선호 기반 점진적 개선**: 과거 브라우저 사용 패턴, 에이전트 요청 이력, 수행 결과 피드백을 누적해 사용할수록 더 정확하고 빠르게 태스크가 처리되는지
  - **에이전트 동작 실시간 가시성**: 에이전트가 현재 어떤 단계를 수행 중인지(페이지 분석 중, 정보 입력 중, 결과 대기 중 등)를 사용자가 실시간으로 확인할 수 있는지
  - **멀티태스킹**: 에이전트가 긴 태스크(구매, 예약 등)를 백그라운드에서 처리하는 동안, 사용자가 TV의 다른 기능(VOD 시청, 채널 변경 등)을 계속 사용할 수 있는지
  - **네트워크 불안정 대응**: Wi-Fi 연결이 일시적으로 끊기거나 불안정할 때 진행 중인 태스크가 graceful하게 처리되고, 재연결 후 이어서 완료할 수 있는지
  - **에이전트 권한 범위 사용자 제어**: 에이전트가 대신 처리할 수 있는 범위를 사용자가 직접 설정할 수 있는지 (예: 구매 한도 금액, 특정 사이트 허용·차단, 자동 완료 허용 태스크 유형)
  - **TV 부적합 페이지의 자동 재구성 (Generative Web Page)**: 데스크톱·모바일 위주로 설계된 웹 페이지가 TV 화면에서 가독성·조작성이 떨어질 때, 시스템이 핵심 콘텐츠를 추출해 TV·리모컨 친화적 레이아웃(OTT 앱 스타일의 D-pad 포커스 네비게이션)으로 재구성된 뷰를 제공하고, 사용자가 필요 시 원본으로 즉시 전환할 수 있는지
- **관련 Architectural View**
...
- **주요 Concern**
  ...
  - **복잡한 웹 페이지의 단순화된 재구성 뷰 (GWP)**: 작은 글자·복잡한 메뉴·다단 레이아웃의 원본 페이지 대신, 큰 카드·단순한 D-pad 포커스 흐름으로 재구성된 뷰를 기본 제공받는지
- **관련 Architectural View**
...
| Architectural View | VD 경영진 | VD 상품화 | Tizen Platform | Web Engine | Security&Privacy | UX/Design | QA/Cert. | 개발팀 | 웹앱 개발자 | 제3자 서비스 | 외부Agent | 일반 시청자 | 고령 사용자 | 접근성 사용자 | 가족 공유 | 규제·법률 |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| Context View | ● | ● | ● | ○ | ○ | ○ | ○ | ○ | ○ | ○ | ● | ○ | ○ | ○ | ○ | ○ |
| Logical View | ○ | ○ | ● | ● | ● | ● | ○ | ○ | ● | ○ | ● | ○ | ○ | ○ | ○ | ○ |
| Component View |  |  | ● | ● | ● | ● | ● | ● | ● | ○ | ○ |  |  |  |  |  |
...
|---|---|---|
| High Power / High Interest | **Manage Closely** — 주요 결정에 선제적 참여, 빈번한 합의·리뷰 | VD 경영진(3.2.1), VD 상품화 담당자(3.2.2), Tizen Platform Team(3.2.3), UX · Design Team(3.2.6) |
| High Power / Low Interest | **Keep Satisfied** — 일상 참여는 제한되나 이슈·게이트 시점에 명확한 승인·검토 요청 | Security & Privacy Team(3.2.5), 규제 · 법률 기관(3.5) |
| Low Power / High Interest | **Keep Informed** — 결정 이유와 변경을 투명하게 공유, 피드백 채널 유지 | Web Engine Team(3.2.4), QA · Certification Team(3.2.7), Tizen Web App 개발자(3.3.1), End User — 일반 시청자(3.4.1), 고령(3.4.2), 접근성(3.4.3), 가족 공유(3.4.4) |
...
```
   ...
   │  • Tizen Platform Team  │  • Tizen 웹앱 개발자    │
...
- **Manage Closely** 주체들은 본 시스템의 범위·설계·출시에 직접 결정권을 행사하거나(VD 경영진·상품화), 본 시스템의 호스트이자 주요 소비자(Tizen Platform Team)로서 일상 합의가 필요하다. UX · Design Team은 Generative UI라는 본 시스템 고유 산출물의 품질과 브랜드 일관성에 대한 소유권을 가지므로 High Power로 취급한다.
- **Keep Satisfied** 주체들은 보안·프라이버시·규제 게이트에서 강한 거부권을 가진다. 그러나 일상 설계 회의에 상시 참여하지 않으므로, 마일스톤·감사 시점의 **명확한 산출물·근거 제공**이 핵심 전략이다.
...
| # | 활동 | VD 경영진 | VD 상품화 | 아키텍트 | Tizen Platform | Web Engine | Security & Privacy | UX / Design | QA / Cert. | 개발팀 |
|---|---|---|---|---|---|---|---|---|---|---|
| 1 | 시스템 경계 · Logical / Component 정의 | I | I | **A**, R | C | C | C | C | I | R |
| 2 | Model Abstraction Layer 인터페이스 | I | I | **A**, R | C |   | C |   | I | R |
| 3 | CDP · Browser Session 계약 |   |   | **A**, R | C | R | C |   | I | R |
| 4 | 호스트(Main Agent / GenUI Renderer) 호출 계약 |   | I | **A**, R | R |   | C | C | I | R |
| 5 | 리소스 예산 · 배치 | C | C | R | **A**, R | C |   |   | I | C |
| 6 | 보안 · 위협 모델 · 권한 모델 |   | I | R | C | C | **A**, R |   | C | R |
| 7 | 프라이버시 · 데이터 보존 |   | I | R | C |   | **A**, R |   | I | R |
| 8 | 동의 · 승인 플로우 확정 |   | I | R | I |   | **A**, R | R | C | R |
| 9 | Generative UI 디자인 시스템 |   | I | R | C | C |   | **A**, R | I | R |
| 10 | 접근성 요건 |   | I | R | I |   | C | **A**, R | C | R |
| 11 | 테스트 전략 · 관측성 |   | I | R | C | C | C | C | **A**, R | R |
| 12 | 로드맵 · 라인업별 탑재 범위 | C | **A**, R | R | C | I | I | I | C | I |
| 13 | 상품화 게이트 · 출시 승인 | **A** | R | C | I | I | C | I | C | I |
| 14 | 개인화·사용자 히스토리 관리 설계 | I | I | **A**, R | C |   | C | C | I | R |
...
| 제3자 웹 서비스 운영자 (3.3.2) | 활동 3 (에이전트 접근 정책), 활동 8 (동의·승인 플로우) | I — 정책 모니터링 및 변경 통보 |
| 외부 Agent 플랫폼 통합 개발자 (3.3.3) | 활동 1 (시스템 경계·인터페이스 정의), 활동 4 (호출 계약), 활동 6 (보안·권한) | C — Sub-Agent / Skill 호출 인터페이스 요구사항 및 인증 방식 자문 |
| 규제 · 법률 기관 (3.5) | 활동 6~8, 10, 13, 14 | I — 준수 증거 제출 및 감사 대응 |
