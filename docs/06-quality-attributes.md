...
| 시나리오 | NFR | 결정 영역 (예측) |
|---------|-----|------|
| 단순 명령 1초 이내 응답 (Quick Action) | NFR-001 | 에이전트 루프 우회 경로 설계 (Quick Action 라우터) |
| 슬립 복귀 1초 이내 가용 | NFR-005 | Warm-start / pre-warming 전략, 하네스 생명주기 |
| 복잡 태스크 카테고리별 완료 시간 한도 | NFR-030 | 시나리오별 모델·도구 선택 정책, 병렬 도구 호출 |
| 메모리 풋프린트 라인업별 예산 준수 | NFR-006 | 모델 추상화 전략, 라인업별 기능 차등 |
| 세션당 토큰 비용 예산 준수 | NFR-008 | 컨텍스트 압축, 모델 라우팅(small/large), 캐싱 |
| 에이전트 오류의 상위 시스템 격리 (≥99.95%) | NFR-009 | 프로세스 격리·샌드박스 경계, 폴백 전략 |
| 프롬프트 인젝션 탐지 (벤치마크 기반) | NFR-014 | 입력 검증 파이프라인, 외부 콘텐츠 신뢰 모델 |
| 의도 해석 정확도 ≥90% | NFR-020 | NLU 전략 (LLM-기반 vs 하이브리드) |
| 태스크 성공률 ≥80% | NFR-021 | 에이전트 루프 설계, HITL 적용 기준, 시나리오 카탈로그 |
| 시뮬레이션 커버리지·결정성 (≥80% / ≥95%) | NFR-025 | 테스트 인프라, 외부 의존성 모킹 전략 |
