...
│   │
│   └── GWP 변환 시맨틱 보존
│       └── 핵심 액션 보존율 100% / 콘텐츠 보존율 ≥ 임계값        [H, H]  → NFR-033
│
├── Accessibility (접근성)
│   │
│   └── 표준 준수
│       └── WCAG 2.2 AA 위반 = 0 (GenUI 인터페이스 산출물 + GWP 변환 페이지 포함) [H, M]  → NFR-022
│
...
본 시스템 설계의 **아키텍처 결정 핵심 동인**이 되는 12개 leaf다. 7장(Solution Strategy)·9장(ADR)에서 우선 다뤄야 할 항목이다.
...
- NFR-031 GWP 변환 지연 / NFR-032 GWP D-pad 포커스 응답
...
| TO-3 | 자원 예산(메모리·토큰) ↔ 태스크 정확도 | 모델 라우팅·컨텍스트 압축률·캐시 | 7장 / ADR(모델 추상화 전략) |
...
- **긴장 관계**: 큰 모델·긴 컨텍스트·캐시는 정확도를 높이나 메모리·토큰·비용 예산을 초과한다. 모델 라우팅(small/large)·컨텍스트 압축으로 절감하면 정확도 변동 위험.
- **민감 파라미터**: 모델 선택(라우팅) 정책 + 컨텍스트 압축률 + 캐시 크기.
- **현재 방향(가설)**: 난이도 기반 모델 라우팅(단순=경량/Quick Action, 복잡=대형), 컨텍스트 압축, 응답 캐싱. 50MB 고정 예산(전 라인업 공통)상 온디바이스 모델 가중치 상주를 최소화하고 클라우드 LLM을 1차로 둠(TC-002와 정합).
- **관련 [H,H] leaf**: NFR-006, NFR-008, NFR-021
- **해소 위치**: 7장(모델 추상화·라우팅·압축·캐싱) / 9장 ADR(모델 전략)
...
1. **트레이드오프 정량 균형점** — TO-1~7 각각의 균형 수치(예: 마스킹 적극성 대비 정확도 손실 허용폭, 모델 라우팅 임계값, 격리 경계 수)는 7장·9장에서 실측·결정한다. 본 장은 충돌 구조까지만 확정.
