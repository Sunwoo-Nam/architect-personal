...
│   │
│   ├── 격리성 (Isolation)
│   │   └── 에이전트 오류의 상위 시스템 영향 격리 (≥99.95%)     [H, M]  → NFR-009
│   │
│   └── 복원력 (Resilience)
│       └── 네트워크 끊김 후 30초 내 재개 성공률 ≥90%           [M, H]  → NFR-010
│
├── Security (보안)
│   │
│   ├── 경계 통제 (Boundary Control)
│   │   ├── 샌드박스 경계 위반 = 0 / 권한 최소화     [H, L]  → NFR-012
│   │   └── 외부 호출자 인증·권한 검증 (무인증 거부 = 100%)     [M, H]  → NFR-015
│   │
│   ├── 공격 표면 방어 (Attack Surface Defense)
│   │   └── 프롬프트 인젝션 탐지율 (벤치마크 기반)              [H, H]  → NFR-014
│   │
│   └── 사용자 보호 (User Safety)
│       └── 보호자 제한 모드 유해 콘텐츠·구매 차단              [H, M]  → NFR-036
│
├── Privacy (프라이버시)
│   │
...
│   │   └── PII 기본 보존 90일, 만료 후 24시간 내 삭제          [M, L]  → NFR-017
│   │
...
│   │
│   ├── 의도 해석 정확도
│   │   └── 표준 시나리오 의도 분류 정확도 ≥90%                 [H, M]  → NFR-020
│   │
│   └── 태스크 수행 정확도
...
│   │
│   ├── 모듈 결합도
│   │   └── 영역 간 비공개 심볼 참조 = 0                        [H, M]  → NFR-024
│   │
│   └── 배포성 (Deployability)
│       └── 펌웨어와 독립적 다운로드·롤백 배포                  [L, M]  → NFR-035
│
├── Testability (테스트 가능성)
│   │
│   ├── 결정적 재현
│   │   └── Top-100 시나리오 시뮬레이션 커버리지 ≥80%, 결정성 ≥95%  [M, H]  → NFR-025
│   │
│   └── 격리 테스트
...
└── Observability (관측성)
    │
    └── 추적성 (Traceability)
        └── 에이전트 트레이스 보존 ≥30일, 조회 P95 ≤5s          [L, L]  → NFR-027
```
...
본 시스템 설계의 **아키텍처 결정 핵심 동인**이 되는 6개 leaf다. 7장(Solution Strategy)·9장(ADR)에서 우선 다뤄야 할 항목이다. *(2026-07 우선순위 재조정으로 11개 → 6개로 압축 — 재조정 전 핵심 후보였던 NFR-005·009·020·025·034는 6.3.2~6.3.3으로 이동.)*
...
### 6.3.2 [H, M]·[H, L] — 표준 기법 적용 영역 (High value, Established)

품질 속성 가치는 높지만 산업 표준 기법으로 달성 가능한 영역. 표준 적용 + 검증 게이트가 핵심.

- [H, M]: NFR-009 오류 격리 / NFR-018 PII 마스킹 / NFR-020 의도 정확도 / NFR-022 WCAG 2.2 AA / NFR-024 모듈 결합도 / NFR-036 보호자 제한 모드 차단 / NFR-037 표준 규약 상호운용성
- [H, L]: NFR-007 CPU 점유율 / NFR-012 샌드박스·권한최소화 / NFR-034 사용자 개입 중지

### 6.3.3 [M, H] — 고난이도 관찰 영역 (Watch)

중요도는 중간이나 기술적 달성 난이도가 높아, 표준 운영에 두기보다 기술 검증(스파이크)과 리스크 모니터링(11장 연계)이 필요한 항목.

- NFR-010 네트워크 재개 / NFR-015 외부 인증 / NFR-025 시뮬레이션 커버리지·결정성

### 6.3.4 [M, M] 이하 — 표준 운영 영역

표준 도구·관행으로 달성하며, 본 장 우선순위는 낮지만 12장(로드맵)·운영 단계에서 챙겨야 할 항목.

- [M, M]: NFR-004 Skill 라우팅 / NFR-005 슬립 복귀 / NFR-011 SKU 성공률 동등성 / NFR-029 저장소
- [M, L]: NFR-002 첫 피드백 / NFR-003 진행 표시 / NFR-017 PII 보존 / NFR-019 데이터 주체 권리 / NFR-023 인터페이스 호환성 / NFR-026 Mock / NFR-028 HITL 재개
- [L, *]: NFR-027 트레이스 [L, L] / NFR-035 독립 배포 [L, M]
...
2026-07 우선순위 재조정으로 [H, H] 핵심군은 11개에서 6개로 압축되었다 — 표준 기법으로 달성 가능하거나 난이도 재평가된 항목을 [H, M|L]·[M, H] 그룹으로 재배치한 결과다. 이 6개(Quick Action 응답성 · 완료 시간 · 메모리 · 토큰 예산 · 인젝션 방어 · 태스크 성공률)가 7~9장 핵심 결정의 입력이 되며, [M, H] 3건(네트워크 재개 · 외부 인증 · 시뮬레이션)은 기술 검증·리스크 관찰 대상으로 11장(리스크)과 연계한다.
...
- **6.6 QA 시나리오 카탈로그** — 전체 32개 leaf를 SEI 6-part 형식으로 풀어 쓰고(우선순위 4그룹, QAS-001~032), ATAM 워크숍 입력으로 사용. *(작성됨, 아래 6.6 참조)*
...
본 절은 6.2.2 Utility Tree의 **전체 32개 leaf**를 SEI 6-part 형식(Source / Stimulus / Artifact / Environment / Response / Response Measure)으로 풀어 쓴다. 각 시나리오는 5장 NFR을 정량 기준으로 인용하며, 8장(Views)·9장(ADR)·10장(ATAM)의 직접 입력으로 사용된다. 시나리오는 6.3의 우선순위 분류를 따라 네 그룹으로 배치하며, QAS 번호와 leaf(NFR)는 1:1로 대응한다. QAS 번호는 append-only로 부여한다 — 신설 leaf는 마지막 번호 다음을 받고(예: QAS-031), 그룹 배치는 6.3 우선순위를 따른다. 2026-07 우선순위 재조정 이후 번호 순서와 그룹 배치는 일치하지 않는다(번호는 이력 보존, 배치는 현재 우선순위).

- **6.6.1 [H, H] 핵심 시나리오** — QAS-001·003·004·005·007·009 (6.3.1의 아키텍처 결정 핵심 후보)
- **6.6.2 [H, M]·[H, L] 시나리오** — QAS-006·008·011·013·015·018·019·027·031·032 (6.3.2의 표준 기법 적용 영역)
- **6.6.3 [M, H] 시나리오** — QAS-010·014·016 (6.3.3의 고난이도 관찰 영역)
- **6.6.4 표준 운영 시나리오** — QAS-002·012·017·020~026·028~030 (6.3.4의 표준 운영 영역)

표기:
- **Mapped NFR**: 본 시나리오를 정량화하는 NFR ID
- **Mapped QA leaf**: 6.2.2 Utility Tree 상의 위치 (Quality Attribute — 정련)

네 그룹 모두 동일한 6-part 형식으로 기술하여 ATAM 워크숍에서 우선순위와 무관하게 참조 가능하게 한다. 단, **[H, H] 그룹만 7·9장 핵심 결정의 1차 입력**이 되며, 나머지 그룹은 표준 기법·운영·리스크 관찰로 달성한다(6.3 참조). TBD 수치는 해당 NFR·ADR 위치를 그대로 인용한다.

---

### 6.6.1 [H, H] 핵심 시나리오

6.3.1의 아키텍처 결정 핵심 후보 6개다.
...
품질 가치는 높으나 산업 표준 기법으로 달성 가능한 10개다([H, M] 7 · [H, L] 3 — 6.3.2). 표준 적용 + 검증 게이트가 핵심이며, 아키텍처 결정을 새로 만들기보다 기성 패턴을 정확히 적용하는 것이 목표다.
...
- **Mapped NFR**: NFR-009
- **Mapped QA leaf**: Availability — 격리성 [H, M]
...
- **Mapped NFR**: NFR-020
- **Mapped QA leaf**: Functional Quality — 의도 정확도 [H, M]
...
- **Mapped NFR**: NFR-034
- **Mapped QA leaf**: Performance — 응답성 (사용자 개입 중지) [H, L]
...
- **Mapped NFR**: NFR-007
- **Mapped QA leaf**: Performance — 자원 효율 [H, L]
...
- **Mapped NFR**: NFR-012
- **Mapped QA leaf**: Security — 경계 통제 [H, L]
...
- **Mapped NFR**: NFR-024
- **Mapped QA leaf**: Modifiability — 모듈 결합도 [H, M]

---

#### QAS-031 보호자 제한 모드 콘텐츠·구매 차단

- **Source**: 사용자 (가구 공유 환경의 구성원 — 자녀 포함), TV 플랫폼 (보호자 제한 설정 상태)
- **Stimulus**: 보호자 제한 모드 활성 상태에서 등급 초과(유해) 콘텐츠 접근 또는 구매·구독·결제로 이어지는 태스크가 요청됨 (직접 요청, 다단계 태스크 중 간접 도달, 외부 콘텐츠의 프롬프트 인젝션 유도 포함)
- **Artifact**: 태스크 계획·실행 경로(에이전트 루프·Quick Action), 콘텐츠 등급 필터, 구매 차단 게이트, 플랫폼 보호자 제한 설정 인터페이스
- **Environment**: 플랫폼 보호자 제한 모드 활성(연령 등급 설정됨), 정상 운영
- **Response**: 시스템이 해당 접근·태스크를 거부 또는 중단하고 사유를 통지하며, 에이전트를 통한 제한 해제 경로를 제공하지 않음
- **Response Measure**: 구매·구독·결제 차단 우회 = **0건**, 등급 초과 콘텐츠 접근 차단율 ≥ **TBD %** (벤치마크 셋 확정 후), 에이전트 경유 제한 해제 = **0건** (NFR-036 참조)
- **Mapped NFR**: NFR-036
- **Mapped QA leaf**: Security — 사용자 보호 [H, M]

> QAS-031은 QAS-030 이후 신설되어 append-only 번호를 받았으며, 우선순위 그룹([H, M])에 따라 본 절에 배치되었다 (6.6 서두 표기 원칙 참조).

---

#### QAS-032 외부 모듈 표준 규약 연결·교체

- **Source**: ARGO 개발팀 (Main Agent 소유자 — Skill 등록·교체), SW 개발팀 (도구·Skill 통합)
- **Stimulus**: 표준 규약을 준수하는 신규 또는 대체 외부 모듈(호출자·도구)을 연결하거나 기존 모듈과 교체함
- **Artifact**: Skill/Sub-Agent 호출 인터페이스, 도구·Skill 연동 경계, 모듈 등록·설정 메커니즘
- **Environment**: 개발·통합 환경, 표준 규약(ADR 선정) 적합성 테스트 셋 구성됨 (LLM·CDP 경계는 적용 제외 — TC-001/002)
- **Response**: 시스템이 코드 변경 없이 등록·설정만으로 연동을 수립하고, 기존 태스크가 정상 동작함
- **Response Measure**: 규약 적합성 테스트 통과 = **100%**, 연결·교체 시 본 시스템 코드 변경 = **0건**, 교체 후 표준 시나리오 회귀 통과 = **100%** (NFR-037 참조)
- **Mapped NFR**: NFR-037
- **Mapped QA leaf**: Compatibility — 상호운용성 [H, M]

---

### 6.6.3 [M, H] 시나리오

중요도는 중간이나 기술적 달성 난이도가 높아 기술 검증(스파이크)·리스크 관찰 대상인 3개다(6.3.3). 11장(리스크)과 연계한다.
...
- **Mapped NFR**: NFR-025
- **Mapped QA leaf**: Testability — 결정적 재현 [M, H]
...
- **Mapped NFR**: NFR-010
- **Mapped QA leaf**: Availability — 복원력 [M, H]
...
- **Mapped NFR**: NFR-015
- **Mapped QA leaf**: Security — 경계 통제 [M, H]

---

### 6.6.4 표준 운영 시나리오

표준 도구·관행으로 달성하며 본 장 우선순위는 낮으나, 12장(로드맵)·운영 단계에서 챙겨야 할 13개다(6.3.4). ATAM 워크숍의 완전성 확보를 위해 동일 형식으로 기술한다.
...
- **Mapped NFR**: NFR-017
- **Mapped QA leaf**: Privacy — 보존 통제 [M, L]
...
- **Mapped NFR**: NFR-035
- **Mapped QA leaf**: Modifiability — 배포성 [L, M]
...
- **Mapped NFR**: NFR-027
- **Mapped QA leaf**: Observability — 추적성 [L, L]
...
- **관련 leaf**: NFR-021 [H,H] · NFR-020 [H,M] / 관련 NFR-018
...
- **관련 leaf**: NFR-009 [H,M] / 관련 NFR-012, NFR-005, NFR-006 [H,H]
...
- **관련 leaf**: NFR-021 [H,H] · NFR-020 [H,M] / 관련 NFR-017, NFR-018, TC-002
...
- **관련 leaf**: NFR-025 [M,H] / 관련 NFR-021
