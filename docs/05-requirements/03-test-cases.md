...
> **개정 이력 (2026-07-11 UC 재번호)**: [use-cases.md](01-functional/use-cases.md) 전면 재번호(UC-01~23, 결번 제거)에 맞춰 Part 1의 TC-UC를 **TC-UC-01~23**으로 재배정했다(UC와 1:1 대응). Part 2의 TC-NFR은 NFR 번호를 그대로 따르며, 삭제분(**TC-NFR-13·16·31·32·33**)은 결번을 유지한다.
...
### TC-UC-08 — UC-08 에이전트 행동 설명 요청 (Explainability)
...
### TC-UC-09 — UC-09 개인화 — 히스토리·선호 누적 및 활용
...
### TC-UC-11 — UC-11 Quick Action — 에이전트 루프 우회 빠른 경로 실행
...
### TC-UC-12 — UC-12 전원 상태 변화에 따른 에이전트 생명주기 관리
...
### TC-UC-13 — UC-13 에이전트 오류 격리 및 상위 시스템 보호
...
### TC-UC-14 — UC-14 자격증명 비노출 사용 및 권한 최소화
...
### TC-UC-15 — UC-15 프롬프트 인젝션 탐지 및 방어
...
### TC-UC-16 — UC-16 PII 분류·보존 기간 제어 및 자동 삭제
...
### TC-UC-17 — UC-17 개인정보 동의 수집 및 데이터 주체 권리 행사
...
### TC-UC-18 — UC-18 데이터 처리 방식 사용자 고지 (온디바이스 vs 클라우드)
...
### TC-UC-19 — UC-19 argot Main Agent의 Sub-Agent / Skill 호출
...
### TC-UC-20 — UC-20 공개 인터페이스 버전 관리 및 하위 호환성
...
### TC-UC-21 — UC-21 제3자 웹 서비스 접근 — 신원 표명 및 정책 준수
...
### TC-UC-22 — UC-22 개발자 디버깅 — 실행 트레이스 및 계측 조회
...
### TC-UC-23 — UC-23 시뮬레이션·Mock 기반 격리 테스트
