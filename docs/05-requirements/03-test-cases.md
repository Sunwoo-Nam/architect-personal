...
### TC-UC-08 — UC-08 Generative Web Page (GWP)로 페이지 재구성 및 표시
- **Test Feature**: GWP 변환 시 핵심 콘텐츠·액션 보존, 디자인 토큰 준수, 접근성 속성, D-pad 포커스 네비게이션 동작
- **Pre-Condition**: GWP 트리거(자율·에이전트 매개·사용자 명시 중 하나)가 발생하여 대상 페이지에 GWP가 적용됨
- **InputSpec + Procedure**:
  1. 표준 페이지 코퍼스(쇼핑·뉴스·예약 각 N개)에 대해 GWP 변환을 수행한다.
  2. 변환된 DOM의 핵심 콘텐츠·액션 가능 요소 보존을 원본과 비교 검증한다.
  3. 렌더링된 컴포넌트의 디자인 토큰·접근성 속성을 검사한다.
  4. D-pad 방향키로 포커스를 이동하고 OK 키로 액션을 트리거한다.
- **Output Spec**: ① 핵심 액션 가능 요소(폼·결제 버튼·필수 링크) 보존율 = 100%, 핵심 콘텐츠 보존율 ≥ NFR-033 임계값 ② 모든 컴포넌트가 Tizen TV 디자인 토큰을 준수하고 접근성 속성(role·label·description·state)을 포함 ③ D-pad 입력에 대한 포커스 응답 시간 ≤ NFR-032 한도 ④ 변환 추가 지연 ≤ NFR-031 한도
...
- **Pre-Condition**: 외부 플랫폼이 표준 인터페이스로 시스템을 등록함
- **InputSpec + Procedure**:
...
### TC-UC-28 — UC-28 GWP 트리거 — 자율·에이전트 매개·사용자 명시
- **Test Feature**: GWP 3개 트리거 경로 동작 및 적합성 판정
- **Pre-Condition**: GWP 서비스 활성, 적합성 판정 기준 운영자 설정값 적용
- **InputSpec + Procedure**:
  1. **자율 트리거**: TV 부적합 페이지(데스크톱 위주 레이아웃) 10건과 적합 페이지(이미 TV/RWD 친화) 10건을 로드하여 GWP 적용 여부를 관찰한다.
  2. **에이전트 매개 트리거**: 에이전트가 검색 결과 페이지를 사용자에게 제시할 때 사용자 의도 힌트(예: 가격 비교)를 함께 전달하고, 레이아웃 강조가 힌트에 부합하는지 검증한다.
  3. **사용자 명시 트리거**: TV 적합 페이지에서 "이 페이지 TV용으로 보여줘" 발화로 GWP 변환이 즉시 수행되는지 확인한다.
  4. 자율 트리거 케이스의 변환 추가 지연을 측정한다.
- **Output Spec**: ① 자율 트리거: 부적합 페이지에 대해 GWP 적용/제안, 적합 페이지는 미적용 ② 에이전트 매개: 힌트가 레이아웃 우선순위·강조에 반영 ③ 사용자 명시: 적합성 판정과 무관하게 즉시 변환 ④ 변환 추가 지연 ≤ NFR-031 한도

### TC-UC-29 — UC-29 원본 ↔ GWP 토글 (원본 보존)
- **Test Feature**: 원본/GWP 토글 동작 및 사용자 선호 저장
- **Pre-Condition**: GWP가 적용된 페이지가 표시되고 있음
- **InputSpec + Procedure**:
  1. 토글 어포던스를 통해 원본 → GWP → 원본을 반복 전환한다.
  2. 페이지 재로드 발생 여부와 스크롤 위치·인터랙션 상태 유지를 검증한다.
  3. 특정 사이트에 대해 "원본으로만 보여줘" 선호를 설정한 뒤, 동일 사이트 재방문 시 자율 GWP가 적용되지 않는지 확인한다.
- **Output Spec**: ① 토글 시 재로드 없음 ② 가능한 경우 동등 위치로 매핑되어 상태 보존 ③ 사용자 선호가 저장되어 이후 자율 트리거가 억제됨
...
### TC-NFR-31 — NFR-031 GWP 변환 추가 지연 시간
- **Test Feature**: 페이지 로드 시 GWP가 야기하는 추가 지연
- **Pre-Condition**: 표준 페이지 코퍼스 및 정상 네트워크 조건
- **InputSpec + Procedure**: ① 코퍼스의 적합·부적합 페이지를 각 30회 로드하여 적합성 판정 자체의 추가 지연을 측정 ② 부적합 페이지 30회에 대해 GWP 적용 시 원본 대비 첫 픽셀까지의 추가 지연을 측정
- **Output Spec**: 적합성 판정 추가 지연 P95 ≤ 300ms, GWP 적용 시 첫 픽셀 추가 지연 P95 ≤ 1.5초.

### TC-NFR-32 — NFR-032 GWP D-pad 포커스 응답 시간
- **Test Feature**: D-pad 입력 후 포커스 시각 피드백 갱신 시간
- **Pre-Condition**: GWP 렌더링된 페이지 표시 중, 입력 이벤트 타임스탬프 계측 가능
- **InputSpec + Procedure**: 포커스 가능 카드 그리드에서 방향키 100회 입력 시 키 이벤트 ~ 포커스 인디케이터 화면 갱신 시간을 측정한다.
- **Output Spec**: 포커스 응답 시간 P95 ≤ 100ms.

### TC-NFR-33 — NFR-033 GWP 변환 콘텐츠의 시맨틱 보존 정확도
- **Test Feature**: 변환 결과의 핵심 콘텐츠·액션 보존
- **Pre-Condition**: 표준 페이지 코퍼스(쇼핑·뉴스·예약 각 N개) 및 수동 평가 절차 정의
- **InputSpec + Procedure**: ① 각 페이지를 GWP로 변환하고 변환된 DOM에서 원본의 핵심 액션 가능 요소(폼·결제 버튼·필수 링크)가 모두 존재하며 원본 시맨틱을 유지하는지 자동 검증 ② 핵심 콘텐츠 보존율은 수동 평가로 측정
- **Output Spec**: 핵심 액션 가능 요소 보존율 = 100% (누락 시 회귀), 핵심 콘텐츠 보존율 ≥ 임계값(TBD).
